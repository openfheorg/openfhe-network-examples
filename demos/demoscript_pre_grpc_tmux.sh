...
usage()
{
    echo "usage: demoscript_pre_grpc_tmux.sh [[-i(ternactive)] [-m(odel) INDCPA|FIXED_NOISE_HRA|NOISE_FLOODING_HRA|NOISE_FLOODING_HRA_HYBRID ] [-s(sl on)] | [-h]] "
}
...
if [[ "$interactive_on" -ne 1 ]] ; then sleep 2; else read -p "Hit any key>" ; printf "\n"; fi
...
if [[ "$interactive_on" -ne 1 ]] ; then sleep 5; else read -p "Hit any key>" ; printf "\n"; fi
