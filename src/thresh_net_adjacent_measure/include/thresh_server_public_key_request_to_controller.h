 ...
 ***/
#ifndef __THRESH_SERVER_PUBLIC_KEY_TO_CONTROLLER_REQUEST__
#define __THRESH_SERVER_PUBLIC_KEY_TO_CONTROLLER_REQUEST__
...
class PublicKeyToControllerRequestServicer : public CallData<thresh_net::ThreshServer::AsyncService> {
public:
	PublicKeyToControllerRequestServicer(ThreshServer::AsyncService* service, ServerCompletionQueue* cq,
	...
	void Proceed() override {
		if (status_ == CREATE) {
			...
			service_->RequestJointPublicKeyToController(&ctx_, &request_, &responder_, cq_, cq_, this);
		}
		else if (status_ == PROCESS) {
			// Spawn a new PublicKeyToClientRequestServicer instance to serve new clients while we process
			// the one for this PublicKeyToClientRequestServicer. The instance will deallocate itself as
			// part of its FINISH state.
			// TODO (dsuponit): do we really need this "new" instance in every Proceed()??? I am still not sure
			new PublicKeyToControllerRequestServicer(service_, cq_, pkeys, pkeysready);
...
#endif // __THRESH_SERVER_PUBLIC_KEY_TO_CONTROLLER_REQUEST__
