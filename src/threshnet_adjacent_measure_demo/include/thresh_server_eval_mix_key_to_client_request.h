 ...
 ***/
#ifndef __THRESH_SERVER_EVAL_MIX_KEY_TO_CLIENT_REQUEST__
#define __THRESH_SERVER_EVAL_MIX_KEY_TO_CLIENT_REQUEST__
...
class EvalMixMultKeyToClientRequestServicer : public CallData<thresh_net::ThreshServer::AsyncService> {
public:
	EvalMixMultKeyToClientRequestServicer(ThreshServer::AsyncService* service, ServerCompletionQueue* cq,
	...
	void Proceed() override {
		if (status_ == CREATE) {
			...
			service_->RequestevalMixMultKeyToClient(&ctx_, &request_, &responder_, cq_, cq_, this);
		}
			...
			new EvalMixMultKeyToClientRequestServicer(service_, cq_, ekeys, ekeysready);

			//send the correct eval mix mult key from map ekeys by using client id
...
#endif // __THRESH_SERVER_EVAL_MIX_KEY_TO_CLIENT_REQUEST__
