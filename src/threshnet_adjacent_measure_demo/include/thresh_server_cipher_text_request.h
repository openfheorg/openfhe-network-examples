 ...
 ***/
#ifndef __THRESH_SERVER_CIPHER_TEXT_REQUEST__
#define __THRESH_SERVER_CIPHER_TEXT_REQUEST__
...
class CipherTextRequestServicer : public CallData<thresh_net::ThreshServer::AsyncService> {
public:
	CipherTextRequestServicer(ThreshServer::AsyncService* service, ServerCompletionQueue* cq,
		std::unordered_map<uint32_t, lbcrypto::Ciphertext<lbcrypto::DCRTPoly>>& CipherTexts)
		: CallData(service, cq, CREATE), responder_(&ctx_), cts(CipherTexts) {
		Proceed();
	}
	...
	void Proceed() override {
		if (status_ == CREATE) {
			...
			service_->RequestCipherTextFromClient(&ctx_, &request_, &responder_, cq_, cq_, this);
		}
		else if (status_ == PROCESS) {
			new CipherTextRequestServicer(service_, cq_, cts);
			//new CipherTextRequestServicer(service_, cq_, ct, pkey, cc); // for testing
...
private:
    ...
    std::unordered_map<uint32_t, lbcrypto::Ciphertext<lbcrypto::DCRTPoly>>& cts;
	
};

#endif // __THRESH_SERVER_CIPHER_TEXT_REQUEST__
