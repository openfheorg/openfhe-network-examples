 ...
 ***/
#ifndef __KS_BROKER_PUBLIC_KEY_FROM_KS_H__
#define __KS_BROKER_PUBLIC_KEY_FROM_KS_H__
...
class PublicKeyServerRequestServicer : public CallData<pre_net::PreNetServer::AsyncService> {
public:
	PublicKeyServerRequestServicer(pre_net::PreNetServer::AsyncService* service, grpc::ServerCompletionQueue* cq,
		std::unordered_map<pair, lbcrypto::PublicKey<lbcrypto::DCRTPoly>, boost::hash<pair>>& RecdPublicKeys)
		: CallData(service, cq, CREATE), responder_(&ctx_), pkeys(RecdPublicKeys) {
		Proceed();
	}
	...
	void Proceed() override {
		if (status_ == CREATE) {
			status_ = PROCESS;
			service_->RequestPublicKeyfromServer(&ctx_, &request_, &responder_, cq_, cq_, this);
		}
		else if (status_ == PROCESS) {
			new PublicKeyServerRequestServicer(service_, cq_, pkeys);
...
#endif // __PRE_SERVER_PUBLIC_KEY_SERVER_REQUEST__
